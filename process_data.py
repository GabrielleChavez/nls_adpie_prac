                ...
                if re.search(_labels, svc_file) and taskType in CLASS_1:
                    ...
                    for moca in moca_cols:
                        try:
                            ...
                            moca_dict[moca] = float(moca_sessions[session]) if session < len(moca_sessions) else float(moca_sessions[len(moca_sessions) - 1]) # ensure we get moca score for the same
                        except Exception as e:
